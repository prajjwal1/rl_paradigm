...
def train_policy(model):
    ...
    NUM_EPOCHS = 200
    for idx in trange(NUM_EPOCHS):
        #  model.learn(
        #  total_timesteps=TIMESTEPS_PER_ITERATION,
        #  log_interval=TIMESTEPS_PER_ITERATION,
        #  progress_bar=True,
        #  )
        eval_returns = evaluate_policy(model, env, n_eval_episodes=100)
        print(f"Eval return: {eval_returns}")
        #  model.save(f"temp/{idx}_{eval_returns[0]:.2f}")
...
if __name__ == "__main__":
    N_ENVS = 1
    env = make_vec_env("Humanoid-v2", n_envs=N_ENVS)
    model = SAC("MlpPolicy", env, verbose=1)

    #  train_policy(model)
    model = model.load("humanoid_models/sac_medium")  # 5028, 2177
    #  model.load("temp/197_4829.99")
    trajectories: List = []
    avg_return = 0
    for _ in trange(4000):
        trajectory_dict = generate_offline_data(model, env, n_eval_episodes=1)
        trajectories.append(trajectory_dict)
        avg_return += trajectory_dict["rewards"].sum()
    avg_return /= 4000
    print(f"Average Return: {avg_return}")
    # Expert, 5057, 5806

    fname = "/checkpoints/prajj/gym/humanoid-medium-v2.pkl"
