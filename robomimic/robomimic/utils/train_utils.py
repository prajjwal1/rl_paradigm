...
from robomimic.utils.dataset import SequenceDataset, SequenceDatasetDT
...
def dataset_factory(config, obs_keys, filter_by_attribute=None, dataset_path=None):
    ...
    if config.algo_name == "dt":
        dataset = SequenceDatasetDT(**ds_kwargs)
...
def run_rollout_dt(
    ...
    ob_dict = policy._prepare_observation(
        ob_dict
    )  # This calls _prepare_observation of RolloutPolicy
    ob_dict["obs"] = ob_dict
    state = policy.policy.nets["encoder"](**ob_dict)  # Shape is [1, state_dim]
    ...
    try:
        for step_i in range(horizon):
            ob_dict, reward, done, _ = env.step(
                sampled_action
            )  # sampled_action dim is [7]
            ...
            #  print(torch.tensor(np.array(actions)).unsqueeze(0).shape)
...
def rollout_with_stats(
    ...
    for env_name, env in envs.items():
        ...
        for ep_i in iterator:
            ...
            if config.algo_name == "dt":
                #  print(f"terminate_on_success: {terminate_on_success}")
                #  print(f"reward shaping: {env.env.reward_shaping}")
                rollout_info = run_rollout_dt(
                    policy=policy,
                    env=env,
                    horizon=horizon,
                    render=render,
                    ...
                    target_return=config.experiment.target_return,
